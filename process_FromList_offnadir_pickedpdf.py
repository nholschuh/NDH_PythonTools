...
def process_FromList_offnadir_pickedpdf(picked_files,orig_fn_list,layer_save, cresis_flag=1, layer_save_type=1, 
    ...
    if isinstance(picked_files,str) == 1:
    ...
    for ind0,fn in enumerate(picked_files):
        ...
        for ind1,frame_fn in enumerate(tqdm(frame_list)):
            ...
            frame_data = loadmat(standard_fns[ind1],['Time','Latitude','Longitude','Elevation','GPS_time'])
            times = frame_data['Time']
                
            original_width = len(frame_data['Latitude'])-1
